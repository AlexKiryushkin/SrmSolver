                                       ...
                                       ElemT t, ElemT dt, unsigned rkStep) const
{
  ...
  for (std::size_t i{}; i < startIdx; ++i)
  {
    ...
    gasStates.at(i) = getLeftGhostValue(problem, x, t, dt, rkStep);
  }

  for (std::size_t i{ gasStates.size() - 1U }; i >= endIdx; --i)
  {
    const auto x = problem.getX(i);
    gasStates.at(i) = getRightGhostValue(problem, x, t, dt, rkStep);
  }
}

class ExactGhostValueSetter : public IGhostValueSetter
{
  GasState getLeftGhostValue(const Problem& problem, ElemT x, ElemT t, ElemT dt, unsigned rkStep) const override
  {
  ...
  GasState getRightGhostValue(const Problem& problem, ElemT x, ElemT t, ElemT dt, unsigned rkStep) const override
  {
    return getLeftGhostValue(problem, x, t, dt, rkStep);
  }
};
