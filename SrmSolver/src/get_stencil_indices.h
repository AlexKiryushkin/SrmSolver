...
template <unsigned order, class ElemT, class ReturnT = kae::Matrix<unsigned, order, order>>
HOST_DEVICE ReturnT getStencilIndicesImpl(const ElemT *      pCurrPhi,
                                          CudaFloat2T<ElemT> scaledSurfacePoint,
                                          CudaFloat2T<ElemT> scaledNormal, unsigned nx)
{
...
HOST_DEVICE ReturnT getStencilIndices(const ElemT *      pCurrPhi,
                                      ...
                                      CudaFloat2T<ElemT> normal, unsigned nx, ElemT hx, ElemT hy)
{
  const CudaFloat2T<ElemT> scaledSurfacePoint{ surfacePoint.x / hx, surfacePoint.y /hy };
  ...
  return impl::getStencilIndicesImpl<order>(pCurrPhi, scaledSurfacePoint, normal, nx);
}
