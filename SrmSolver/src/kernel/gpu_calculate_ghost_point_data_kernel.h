...
template <unsigned order, class ElemT>
                                        ...
                                        unsigned nx, unsigned ny, ElemT hx, ElemT hy)
{
  ...
  const EBoundaryCondition boundaryCondition = shape.getBoundaryCondition(surfacePoint.x ,surfacePoint.y, hx);
...
void calculateGhostPointDataWrapper(thrust::device_ptr<const ElemT>                        pCurrPhi,
    ...
    thrust::device_ptr<kae::Matrix<unsigned, order, order>> pStencilIndices, Shape<ElemT> shape,
    GpuGridT<ElemT> grid)
{
    calculateGhostPointData<order> << <grid.gridSize, grid.blockSize >> >
        (pCurrPhi.get(), pClosestIndices.get(), pBoundaryConditions.get(),
            pNormals.get(), pSurfacePoints.get(), pStencilIndices.get(), shape, grid.nx, grid.ny, grid.hx, grid.hy);
